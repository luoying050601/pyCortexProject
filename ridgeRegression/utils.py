...
def average_values_at_indices(input_list, index_list):
    # Ensure both lists are non-empty
    # if not input_list or not index_list:
    #     return None  # Return None or any appropriate default value

    # Extract values at specified indices
    selected_values = [abs(input_list[index]) for index in index_list if 0 <= index < len(input_list)]

    # Calculate the average value
    if selected_values:
        average_value = non_zero_average(selected_values)
        return average_value
    else:
        return None  # Return None if no valid values were selected
...
def two_vs_two(y_test, preds, _type='pearsonr'):
    """
    1. Each pair of words is compared only once.
    """
    global dsjj, dsij, dsji, dsii
    points = 0
    total_points = 0
    for i in trange(preds.shape[0] - 1):
        ...
        for j in range(i + 1, preds.shape[0]):
            ...
            # st.pearsonr(k_t, k_p)
            if _type == 'pearsonr':
                dsii = st.pearsonr(s_i, s_i_pred)
                dsjj = st.pearsonr(s_j, s_j_pred)
                dsij = st.pearsonr(s_i, s_j_pred)
                dsji = st.pearsonr(s_j, s_i_pred)
            elif _type == 'cosine':
...
def make_print_to_file(_type, path=LOG_DIR):
    ...
    example:
    use  make_print_to_file()   and the   all the information of funtion print ,
    will be write in to a log file
    :return:
    '''
    import os
...
def correlation_with_pvalue(y_true, y_pre):
    corr_list = []
...
def spearmanr_with_pvalue(y_true, y_pre):
    corr_list = []
    for j in range(y_true.shape[1]):
        corr, p = st.spearmanr(y_true[:, j], y_pre[:, j])
        if np.isnan(corr):
            corr = 0
        else:
            if p >= 0.05:
...
def skendalltau_with_pvalue(y_true, y_pre):
    corr_list = []
    for j in range(y_true.shape[1]):
        corr, p = st.kendalltau(y_true[:, j], y_pre[:, j])
        if np.isnan(corr):
            corr = 0
        else:
            if p >= 0.05:
...
def correlation_roi_dict(y_true, y_pre):
    ...
    for (k_t, k_p) in zip(y_true, y_pre):
        ...
        if len(p_list) != 0:
            ...
            #     if a[1][i] >= 0.05:
            #         corr_roi_list[i] = 0
            print(k_t, np.array(p_list).mean())
            roi_corr_dict[k_t] = p_list
...
# type train test
def load_music_pickle_data(pair_path, file, _type):
    X_filename = '128_' + _type + '_X_' + file + '.pickle'
    Y_filename = 'cortex_' + _type + '_Y_' + file + '.pickle'
    with open(pair_path + X_filename, 'rb') as f:
        X = pickle.load(f)
        f.close()
    with open(pair_path + Y_filename, 'rb') as f:
        Y = pickle.load(f)
        f.close()
    return X, Y
