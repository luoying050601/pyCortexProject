...
from ridgeRegression.layer import BertLayer, BertPooler, BertOnlyMLMHead
...
class BrainBertPreTrainedModel(nn.Module):
    ...
    def from_pretrained(cls, config_file, state_dict, *inputs, **kwargs):
        ...
        if not hasattr(model, 'bert') and any(s.startswith('bert.')
            ...
            start_prefix = 'bert.'
        if not hasattr(model, 'brainlm') and any(s.startswith('brainlm.')
                                              for s in state_dict.keys()):
            start_prefix = 'brainlm.'
        if not hasattr(model, 'brainbert') and any(s.startswith('brainbert.')
                                              for s in state_dict.keys()):
            start_prefix = 'brainbert.'
        if not hasattr(model, 'albert') and any(s.startswith('albert.')
                                              for s in state_dict.keys()):
            start_prefix = 'albert.'
        if not hasattr(model, 'base_model') and any(s.startswith('base_model.')
                                              for s in state_dict.keys()):
            start_prefix = 'base_model.'
...
class BrainBertModel(BrainBertPreTrainedModel):
    ...
    def forward(self, brain_feature, input_ids, attention_mask=None,
        ...
        device = input_ids.device if input_ids is not None else torch.device("cuda" if torch.cuda.is_available() else "cpu")
        ...
        embedding_output = self.embedding(input_ids=input_ids,
                                          brain_feature=None, position_ids=position_ids,
